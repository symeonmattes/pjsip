...
- (void) connect:(CDVInvokedUrlCommand*)command{
    ...
    NSLog(@"Connection settings:{user:%@,pass:%@,systemIP:%@,proxyIP:%@}",user,pass,systemIPAddress,proxyIPAddress);
...
- (void) checkarchitecture:(CDVInvokedUrlCommand*)command{
    ...
    NSString *cpu = [pjdiagn checkArchitecture];
    
    pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:cpu];
    [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
}
...
- (void) checkclientip:(CDVInvokedUrlCommand*)command{
    ...
    NSString *ip = [pjdiagn getIPAddress];
    
    if ([ip isEqualToString:@"error"]){
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"NOT_CONNECTED"];
    }else{
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:ip];
    }
...
- (void) getwifissid:(CDVInvokedUrlCommand*)command{
    ...
    NSString *ssid = [pjdiagn getWifiSSID];
    
    if ([ssid isEqualToString:@"error"]){
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"NOT_CONNECTED"];
    }else{
